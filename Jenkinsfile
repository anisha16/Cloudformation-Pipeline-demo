pipeline {
    ...
    stages {
        ...
        stage("Create SNS Topic") {
            steps {
                script {
                    def snsTopicName = sh(script: "cat ${env.CLOUDFORMATION_FOLDER}/sns.yaml | yq -r '.Parameters[] | select(.ParameterKey == \"TopicName\") | .ParameterValue'", returnStdout: true).trim()
                    withCredentials([aws(credentialsId: 'jenkins-cred', accessKeyVariable: 'AWS_ACCESS_KEY_ID', secretKeyVariable: 'AWS_SECRET_ACCESS_KEY')]) {
                        sh "aws cloudformation create-stack --stack-name my-sns-stack --template-body file://${env.CLOUDFORMATION_FOLDER}/sns.yaml --parameters ParameterKey=TopicName,ParameterValue=${snsTopicName}"
                    }
                }
            }
        }
